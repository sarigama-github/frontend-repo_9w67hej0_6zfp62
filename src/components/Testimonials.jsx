...
export default function Testimonials(){
  return (
    <section id="testimonials" className="relative py-20 sm:py-28">
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(900px 400px at 90% 10%, rgba(76,218,208,0.15), transparent 60%)' }}></div>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {quotes.map((q, i) => (
            <motion.figure key={i} className="relative rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl p-6"
              whileHover={{ y: -4 }}>
              <blockquote className="text-slate-300/90 leading-relaxed">“{q.text}”</blockquote>
              <figcaption className="mt-4 text-sm text-slate-300">
                <span className="text-white font-medium" style={{ background: 'linear-gradient(90deg, #4CDAD0, #C3AEEF)', WebkitBackgroundClip: 'text', color: 'transparent' }}>{q.name}</span> — {q.role}
              </figcaption>
            </motion.figure>
          ))}
        </div>
